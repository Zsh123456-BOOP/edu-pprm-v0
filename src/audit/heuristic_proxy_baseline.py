...
from src.audit.common import AUDIT_DIR, BLIND_PATH, HEURISTIC_LABELS_PATH, detect_local_expression_error, validate_audit_label
...
def heuristic_proxy_label(row: dict[str, Any]) -> dict[str, Any]:
...
def write_notes(labels: list[dict[str, Any]]) -> None:
    ...
    text = f"""# Heuristic Proxy Baseline Notes
...
"""
    (AUDIT_DIR / "heuristic_proxy_baseline.notes.md").write_text(text, encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate heuristic proxy baseline labels from blind view")
    parser.add_argument("--input", default=str(BLIND_PATH))
    parser.add_argument("--output", default=str(HEURISTIC_LABELS_PATH))
    args = parser.parse_args()
    rows = read_jsonl_file(__import__("pathlib").Path(args.input))
    labels = [heuristic_proxy_label(row) for row in rows]
    errors = []
    for label in labels:
        label["annotator"] = "heuristic_proxy"
        label_errors = validate_audit_label(label, expected_annotator="heuristic_proxy")
        if label_errors:
